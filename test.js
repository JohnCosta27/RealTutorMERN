...
async function loginMethod() {
  ...
  request.post(
    ...
    (error, response, body) => {
      if (error == null) {
        if (body.error == null) {
          console.log("Test #2 - Unsucessful login (Wrong email) [FAILED]");
        } else {
          console.log("Test #2 - Unsucessful login (Wrong email) [SUCESSFUL]");
        }
      }
    }
  );
  ...
  request.post(
    ...
    (error, response, body) => {
      if (error == null) {
        if (body.error == null) {
          console.log("Test #3 - Unsucessful login (Wrong password) [FAILED]");
        } else {
          console.log(
